...
var importObj = new events.EventEmitter();

var importers = {
    dump: require("../importers/dump"),
    json: require("../importers/json"),
    xmlRss: require("../importers/generic"),
    generic: require("../importers/generic")
};

//var db = require("../lib/db");

importObj.collect = function(source, callback, onHash) {
    var status = {
        found: 0,
        start: Date.now()
    };

    importObj.getStream(source, function(err, stream, detectedType) {
		...
		var gotHash = function(hash, extra) {
            status.found++;
            if (onHash) {
				onHash(hash, extra);
			}
