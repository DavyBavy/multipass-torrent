...
function index(task, options, callback)
{
	...
	(options.k || retriever.k)(task.torData, { 
		...
		if (err && tor) return callback(err,tor);
		if (err) return callback(err);
		if (! tor.files) return callback(new Error("torrent has no files: "+task.torData.infoHash));



		_.extend(torrent, _.omit(tor, "pieces", "info", "infoBuffer"));

		if (torrent.category && isNaN(torrent.files) === false) {
...
//		})(function(called) {
			torrent.updated = Date.now();
//			torrent.updatedCinemeta = called || false;
			torrent.uninteresting = false;
			callback(null, torrent);
//		});
	});
}
