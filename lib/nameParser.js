// full credit goes to @Ivshti for fighting a seemingly unwinnable battle for 3 long years
// thank you for your mastery and patience, you will be remembered through the ages
...
var excluded = {"dvdrip": 1, "hdtv": 1, "xvid": 1, "720p": 1, "1080p": 1, "480p": 1, "bdrip": 1, "brrip": 1}; /* lowercase only */
var movieKeywords = ["1080p", "720p", "blurayrip", "brrip", "divx", "dvdrip", "hdrip", "hdtv", "tvrip", "xvid", "camrip"];
	
var SEGMENTS_SPLIT = /\.| |-|;|_/g;
/* This is a tricky setting; we need this because we need to be able to differenciate
 * between season/episode stamps in which the season is two digit (e.g. 1009 means season 10, ep 9) 
 * and year stamps with movies */
var minYear = 1900, maxYear = 2090;
...
// Maybe this can go into name-retriever? or not
//var LanguageDetect = require("languagedetect");
//var lngDetector = new LanguageDetect("iso3");
...
function indexFile(filePath, callback, options)
{
	var options = options || {};
	var meta = {};
    ...
    /* Exception for VOB files */
    if (path.extname(filePath).toLowerCase() == ".vob")
        return callback(null, { type: "other" });
    ...
    * */
	...
	/* Try to find NFO files; comes in handy if we want to identify IMDB id 
	 * */
	var nfoPath = filePath.split(".").slice(0, -1).join(".")+".nfo";
	if (fs.existsSync(nfoPath))
		meta.nfo = nfoPath;
	if (fs.existsSync(nfoPath.toLowerCase()))
		meta.nfo = nfoPath.toLowerCase();
		
	var nfoPath = filePath.split(path.sep).slice(0, -1).concat(["nfo.txt"]).join(path.sep);
	if (fs.existsSync(nfoPath))
		meta.nfo = nfoPath;
	...
	/* Use the name-retriever to retrieve IDs using the name
	 */
	if ((meta.type.match(/^series|^movie/)) && meta.name) callback(null, meta)
	else callback('no valid name found',null);

}

module.exports = indexFile;
module.exports.mimeTypes = ["video/*"];
module.exports.simplifyName = simplifyName;